...
class EDLEditor(object):
    ...
    def EditFile(self, videofile, editfile, create):
        ...
        while not ExitMainUi:
            MainUi = GUIMain("%s.xml" % __addonname__.replace(".","-") , __addonpath__, 'Default', status=self.status, framelist=self.framelist)
    ...
    def EditGUI(self, Frames, CurrItem):
        RetItem = CurrItem
        Canceled = True
        EditUi = GUIEdit("%s-edit.xml" % __addonname__.replace(".","-") , __addonpath__, 'Default', item=CurrItem, Frames=Frames, status=self.status)
